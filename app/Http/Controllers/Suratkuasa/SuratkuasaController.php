...
class SuratkuasaController extends Controller
{
    ...
    public function __construct(
        protected SuratKuasaService $suratKuasaService,
    ...
    public function index(PendaftaranSuratKuasaDataTable $dataTable)
    {
        ...
        // Testimoni prompt data for users
        $shouldShowTestimoniPrompt = false;
        $testimoniUser = null;

        if (Auth::user()->role == \App\Enum\RoleEnum::User->value) {
            $shouldShowTestimoniPrompt = $this->homeHelper->shouldShowTestimoniPrompt();
            $testimoniUser = TestimoniModel::where('user_id', Auth::id())->first();
        }
        ...
        $data = [
            ...
            'shouldShowTestimoniPrompt' => $shouldShowTestimoniPrompt,
            'testimoniUser' => $testimoniUser,
        ];
