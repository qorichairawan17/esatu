...
use App\Mail\ProfileWarningMail;
...
/**
 * Class AdvokatNonAdvokatService
 *
 * This service handles the business logic for managing "Advokat" and "Non-Advokat" users.
 * It includes creating, updating, and deleting these users, along with handling
 * their profiles and recording audit trails for these actions.
 */
class AdvokatNonAdvokatService
{
    /**
     * Store or update an advokat/non-advokat user record.
     *
    ...
    /**
     * Delete an advokat/non-advokat user record.
     *
     * This method deletes a user and their associated profile, including
    ...
    /**
     * Send a warning email to a user with an invalid profile.
     *
     * @param  string  $id  The encrypted ID of the user.
     * @return JsonResponse
     */
    public function sendWarning($id)
    {
        try {
            ...
            Mail::to($user->email)->queue(new ProfileWarningMail($user));

            AuditTrailService::record('telah mengirim email peringatan pembaruan profil ke: '.$user->name, ['user_id' => $user->id]);

            return response()->json(['success' => true, 'message' => 'Email peringatan berhasil dikirim.']);
        } catch (\Exception $e) {
            Log::error('Error sending warning email to advokat/non advokat: '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);

            return response()->json(['success' => false, 'message' => 'Terjadi kesalahan saat mengirim email.'], 500);
        }
    }
}
