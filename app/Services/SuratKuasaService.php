...
class SuratKuasaService
{
    ...
    public function previewFile($request)
    {
        try {
            ...
            $columnName = JenisDokumenEnum::tryFromKey(strtoupper($jenis_dokumen));
    ...
    {
        try {
            $pendaftaran = $this->findPendaftaranForDeletion($request->id);

            if (! $pendaftaran) {
                return response()->json(['success' => false, 'message' => 'Pendaftaran surat kuasa tidak ditemukan.'], 404);
            }

            if (! $pendaftaran->canBeDeletedBy(Auth::user())) {
                return response()->json(['success' => false, 'message' => 'Kamu tidak memiliki izin untuk menghapus pendaftaran ini.'], 403);
            }

            $id = $pendaftaran->id;
            $idDaftar = $pendaftaran->id_daftar;
            $oldData = $pendaftaran->toArray();

            DB::beginTransaction();

            $this->deleteSuratKuasaStorage($pendaftaran);
            $this->deleteSuratKuasaRelations($pendaftaran);
            ...
            Log::warning('Gagal mendekripsi ID pendaftaran surat kuasa untuk dihapus.', ['id_encrypted' => $request->id, 'error' => $e->getMessage()]);

            return response()->json(['success' => false, 'message' => 'ID pendaftaran surat kuasa tidak valid.'], 400);
        } catch (\Exception $e) {
    ...
    private function findPendaftaranForDeletion(string $encryptedId): ?PendaftaranSuratKuasaModel
    {
        $id = Crypt::decrypt($encryptedId);

        return PendaftaranSuratKuasaModel::with(['register', 'pembayaran', 'pihak'])->find($id);
    }

    private function deleteSuratKuasaStorage(PendaftaranSuratKuasaModel $pendaftaran): void
    {
        $directories = array_unique(array_map('dirname', $this->collectSuratKuasaFilePaths($pendaftaran)));
    ...
    private function collectSuratKuasaFilePaths(PendaftaranSuratKuasaModel $pendaftaran): array
    {
        $filesToDelete = [];
    ...
    private function deleteSuratKuasaRelations(PendaftaranSuratKuasaModel $pendaftaran): void
    {
    ...
    public function destroyRejected()
    {
        try {
            if (Auth::user()->role === RoleEnum::User->value) {
                return response()->json(['success' => false, 'message' => 'Kamu tidak memiliki izin untuk menghapus semua surat kuasa ditolak.'], 403);
            }

            $ditolakSuratKuasa = PendaftaranSuratKuasaModel::where('status', StatusSuratKuasaEnum::Ditolak->value)->get();
            ...
            foreach ($ditolakSuratKuasa as $suratKuasa) {
                ...
                foreach (self::DOCUMENT_COLUMNS as $field) {
