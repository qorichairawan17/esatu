...
class AdministratorService
{
     ...
     *
     * @param  Request  $request  The request object containing administrator data.
    ...
    public function store($request)
    {
        ...
        try {
            if ($id) {
                ...
                if ($user->role === RoleEnum::Superadmin->value && auth()->user()->role !== RoleEnum::Superadmin->value) {
                    return response()->json(['success' => false, 'message' => 'Kamu tidak memiliki izin untuk mengedit Superadministrator.'], 403);
                }

                // Prevent role escalation to Superadmin by non-Superadmins
                if ($validated['role'] === RoleEnum::Superadmin->value && auth()->user()->role !== RoleEnum::Superadmin->value) {
                    return response()->json(['success' => false, 'message' => 'Hanya Superadministrator yang dapat memberikan akses Superadministrator.'], 403);
                }

                // 1. Capture old data for audit trail                $oldUserData = $user->only(['name', 'email', 'role', 'block']);
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            if ($user->role === RoleEnum::Superadmin->value && auth()->user()->role !== RoleEnum::Superadmin->value) {
                return response()->json(['success' => false, 'message' => 'Kamu tidak memiliki izin untuk menghapus Superadministrator.'], 403);
            }
