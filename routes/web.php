...
Route::prefix('pengguna')->middleware([AuthMiddleware::class, ForbiddenForUserMiddleware::class])->group(function () {
    Route::controller(AdvokatNonAdvokatController::class)->group(function () {
        ...
        Route::get('/advokat-non-advokat/detail/{id}/surat-kuasa', 'suratKuasaData')->name('advokat.surat-kuasa-data');
        ...
        Route::post('/advokat-non-advokat/send-warning/{id}', 'sendWarning')->name('advokat.send-warning');
    });
...
Route::prefix('monitoring-evaluasi')->middleware([AuthMiddleware::class, ForbiddenForUserMiddleware::class])->group(function () {
    Route::controller(MonitoringController::class)->group(function () {
        Route::get('/', 'index')->name('monitoring.index');
        Route::get('/chart-data', 'chartData')->name('monitoring.chart-data');
    });
});
...
Route::get('/notifications/{id}/read', function ($id) {
    ...
    $url = $notification->data['url'] ?? route('dashboard.admin');

    return redirect($url);
})->middleware(AuthMiddleware::class)->name('notifications.read');
